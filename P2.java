...
public class AddressBookDAOImplementation implements AddressBookDAO{
    ...
    {   
       try
       {
          conn = DBConnection.getInstance().getConnect();
           ...
           String qry;
           qry = "insert into addressbook13 values ('"+person.getName()+"','"+person.getMob()+"','"+person.getEmail()+"','"+person.getCity()+"')";
    ...
    @Override
     public void removePerson(String name)
     {
         try
         {
           conn = DBConnection.getInstance().getConnect();
           st = conn.createStatement();
           String qry="delete from addressbook13 where name='"+name+"'";
           st.executeUpdate(qry);
           conn.close();
         }
         catch (SQLException e)
    ...
    @Override
     public void updatePerson(Person person,String name)
     {
         try
       {
           ...
           String qry="update addressbook13 set name='"+person.getName()+"',mob='"+person.getMob()+"',email='"+person.getEmail()+"',department='"+person.getCity()+"',where name='"+name+"'";
           st.executeUpdate(qry);
           conn.close();
        }
    ...
    public DefaultListModel getAllNames(NameListPanel namePanel) 
    {
         ...
         conn = DBConnection.getInstance().getConnect();
            ...
            String qry = "select * from addressbook13 order by name asc";
    ...
    public void getSelectedName(DetailViewPanel detailPanel,String selectedName)
    {
        try {
            ...
            Statement stmt;
            stmt = conn.createStatement();
            String qry = "select * from addressbook13 where name = '"+selectedName+"'";
            ResultSet rs;
    ...
    public DefaultListModel getSearchNames(NameListPanel namePanel, String searchcon) {
     ...
     conn = DBConnection.getInstance().getConnect();
        Statement stmt;
             stmt = conn.createStatement();
             searchcon=searchcon+'%';
        String qry = "select * from addressbook13 where name like '"+searchcon+"'";
             try (ResultSet rs = stmt.executeQuery(qry)) {
                 while (rs.next()){
                     String name = rs.getString(1);
                     dlm.addElement(name);
